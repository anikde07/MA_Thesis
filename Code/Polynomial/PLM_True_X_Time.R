...
dml <- parLapply(cl, 1:MC, function(v) {
  ...
  df            = data.frame(time = rep(1:TM, each = N), individual = rep(1:N, times = TM),
                             Y = as.vector(Y_mat), D = as.vector(D_mat), X = as.vector(FE_fcts_mat))
  
  ###########  Estimating  #################
  model_plm <- plm(Y ~ D + X, data = df, index = c("individual", "time"), model = "within", effect = "time")
  beta_hat = summary(model_plm)$coefficients[,1][1]
  se_hat   = summary(model_plm)$coefficients[,2][1]
  ##########################################
...
write.table(confint_matrix, paste0("Output/True_X_Time/Confidence_intervals for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(theta, paste0("Output/True_X_Time/Theta_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(sigma, paste0("Output/True_X_Time/Sigma_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
