...
print(paste0("DML with KSS being used to estimate X and KNN for m_hat. N = ", N, " K = ", K, " MC = ", MC, " T = ", TM))
...
dml <- parLapply(cl, 1:MC, function(v) {
  ...
  cross_fit <- foreach(i = 1:K, .combine = 'list', .multicombine = TRUE, .maxcombine = K) %do% {
    ...
    m_0_hat <- matrix(NA, nrow = TM, ncol = length(eval_indices))
    
    for (j in 1:TM) {
      train_df <- data.frame(d = as.factor(d_train[j,]), x = x_train[j,])
      test_df  <- data.frame(x = x_eval[j,])

      knn_model <- knn3(d ~ x, data = train_df, k = sqrt(length(d_train[j,])))
      
      temp_pred_matrix <- predict(knn_model, newdata = test_df, type = "prob")
      pred_probs <- numeric(nrow(test_df))
      
      if ("1" %in% colnames(temp_pred_matrix)) {
        pred_probs <- temp_pred_matrix[, "1"]
      } else if ("0" %in% colnames(temp_pred_matrix)) {
        pred_probs <- 1 - temp_pred_matrix[, "0"]
      }
      
      pred_probs <- pmin(pmax(pred_probs, 0.01), 0.99)
      m_0_hat[j, ] <- pred_probs
    }
...
write.table(confint_matrix, paste0("Output/KNN/Confidence_intervals for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(theta, paste0("Output/KNN/Theta_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(sigma_squared, paste0("Output/KNN/Sigma_Squared_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
