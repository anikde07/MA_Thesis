...
print(paste0("DML with KSS being used to estimate X and LASSO to estimate m_hat. N = ", N, " K = ", K, " MC = ", MC, " T = ", TM))
...
clusterExport(cl, ls())
clusterEvalQ(cl, c(library("phtt"), library("ffscb"), library("pspline"), library("caret"), library("foreach"), library("plm"), library("glmnet")))
...
dml <- parLapply(cl, 1:MC, function(v) {
  ...
  cross_fit <- foreach(i = 1:K, .combine = 'list', .multicombine = TRUE, .maxcombine = K) %do% {
    ...
    for (j in 1:TM) {
      ...
      y_train_net <- as.numeric(d_train[j,])
      x_train_net <- t(x_train)
      x_test_net  <- t(x_eval)
      ...
      # Predict probabilities at lambda.min or specific lambda (e.g., 0.01)
      pred_probs <- predict(
        lasso_model,
        newx = x_test_net,
        type = "response"
      ...
      # Clean up predictions to avoid extreme values
...
write.table(confint_matrix, paste0("Output/LASSO/Confidence_intervals for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(theta, paste0("Output/LASSO/Theta_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
write.table(sigma_squared, paste0("Output/LASSO/Sigma_Squared_Hat for N = ", N, "T = ", TM, ".csv"), row.names = FALSE, col.names = FALSE, sep = ",")
